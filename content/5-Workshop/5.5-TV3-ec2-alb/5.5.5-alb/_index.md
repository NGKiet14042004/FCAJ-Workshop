...
3. VPC `zerobug-vpc`, select **2 AZs**, map **public subnet-a** and **public subnet-b**.
4. SG: **`zerobug-alb-sg`**.
5. Listener HTTP **:80** → `zerobug-tg`.
6. Copy **DNS name** → parameter table → **hand off to Trinh**.

#### Target Unhealthy

| Situation | Action |
| --- | --- |
| Backend not running yet | **Normal** — note in report, test later |
| App running but still unhealthy | `systemctl status`; EC2-SG 8080 from ALB-SG; outbound All traffic |
| Outbound SG = 0 | Add All traffic → 0.0.0.0/0 (Trí section 5.3.7) |
